"""
    ...
    Copied from - https://github.com/hzxie/Pix2Vox/blob/master/utils/data_transforms.py

    Also includes jitted versions of two transformations to increase the throughput.
"""
...
class CenterCrop(object):
    ...
    def __call__(self, rendering_images, bounding_box=None):
        num_rendering_images = len(rendering_images)
        if num_rendering_images == 0:
            return rendering_images

        crop_size_c = rendering_images[0].shape[2]
        processed_images = np.empty(shape=(num_rendering_images, self.img_size_h, self.img_size_w, crop_size_c))
        for img_idx, img in enumerate(rendering_images):
            ...
            processed_images[img_idx] = processed_image
            # Debug
            # fig = plt.figure()
...
class ColorJitter(object):
    ...
    def __call__(self, rendering_images):
        ...
        processed_images = np.empty(shape=(num_rendering_images, img_height, img_width, img_channels))
...
class RandomBackground(object):
    ...
    def __call__(self, rendering_images):
        return rand_bg(rendering_images, np.array(self.random_bg_color_range))


@njit(cache=True)
def rand_bg(rendering_images: np.ndarray, random_bg_color_range: np.ndarray):
    ...
    bg_color = np.array([
        np.random.randint(random_bg_color_range[0][0], random_bg_color_range[0][1] + 1),
        np.random.randint(random_bg_color_range[1][0], random_bg_color_range[1][1] + 1),
        np.random.randint(random_bg_color_range[2][0], random_bg_color_range[2][1] + 1)
    ]) / 255.
    bg_color = np.expand_dims(bg_color, axis=0)
    bg_color = np.expand_dims(bg_color, axis=0)

    num_rendering_images = len(rendering_images)
    processed_images = np.empty(shape=(num_rendering_images, img_height, img_width, img_channels - 1))
    for img_idx, img in enumerate(rendering_images):
        alpha = (np.expand_dims(img[:, :, 3], axis=2) == 0).astype(np.float32)
        img = img[:, :, :3]
        img = alpha * bg_color + (1 - alpha) * img

        processed_images[img_idx] = img

    return processed_images


class FixedBackground(object):
    def __init__(self):
        pass

    def __call__(self, rendering_images):
        return fixed_bg(rendering_images)


@njit(cache=True)
def fixed_bg(rendering_images: np.ndarray, fixed_bg_color=None):
    if len(rendering_images) == 0:
        return rendering_images
    ...
    # Generate random background
    bg_color = np.array(fixed_bg_color if fixed_bg_color is not None else [255, 255, 255]) / 255.
    bg_color = np.expand_dims(bg_color, axis=0)
    bg_color = np.expand_dims(bg_color, axis=0)
